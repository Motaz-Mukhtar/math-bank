...
const Navbar = () => {
  ...
  const links = [
    ...
    { label: "الترتيب", href: "/#leaderboard" },
    ...(user?.role === "PARENT" ? [{ label: "لوحة التحكم", href: "/parent-dashboard" }] : []),
    ...(user?.role === "ADMIN" ? [{ label: "إدارة النظام", href: "/admin" }] : []),
  ];
      ...
      <div className="container flex items-center justify-between h-16">
        ...
        <div className="hidden md:flex items-center gap-2">
          {user ? (
            <>
              <NotificationsBell />
              <Link to="/profile" className="flex items-center gap-2 bg-primary/5 rounded-full px-4 py-1.5 hover:bg-primary/10 transition-colors">
